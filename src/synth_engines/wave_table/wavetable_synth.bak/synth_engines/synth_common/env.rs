use crate::synth_engines::wave_table::wavetable_synth::{
    common::EnvParam, config::SAMPLE_RATE, ModulationDest,
};
// use log::*;
...
impl ADSR {
    pub fn new() -> Self {
        ...
        Self {
            ...
            phase: 0,
            base_params,
            tweak_env_by: Self::calc_tweak_by(base_params),
            env: 0.0,
        }
    }

    fn calc_tweak_by(base_params: [f32; 5]) -> [f32; 5] {
        let mut tweak_env_by = [0.0; 5];

        tweak_env_by[ATTACK] = Self::calc_atk(base_params[ATTACK]);
        tweak_env_by[DECAY] = Self::calc_decay(base_params[DECAY], base_params[SUSTAIN]);
        tweak_env_by[RELEASE] = Self::calc_release(base_params[RELEASE], base_params[SUSTAIN]);

        // println!("{}", tweak_env_by[DECAY]);
    ...
    pub fn set_atk(&mut self, atk: f32) {
        ...
        self.tweak_env_by[ATTACK] = Self::calc_atk(self.base_params[ATTACK]);
    }
    ...
    pub fn set_release(&mut self, release: f32) {
        ...
        self.tweak_env_by[RELEASE] = Self::calc_release(release, self.base_params[SUSTAIN]);

        // debug!("release: {release}, sustain: {}", self.base_params[SUSTAIN]);
        // debug!("release_tweak_by: {}", self.tweak_env_by[RELEASE]);
    }

    /// used to generate an env sample
    pub fn get_samnple(&mut self) -> f32 {
        self.env += self.tweak_env_by[self.phase];
    ...
    pub fn release(&mut self) {
        self.phase = RELEASE;
        // self.env = self.base_params[SUSTAIN];
        self.tweak_env_by[RELEASE] = Self::calc_release(self.base_params[RELEASE], 1.0 - self.env);
